                            ...
                            <tbody>
                                @foreach ($transaction_types as $transaction_type)
                                    <tr>
                                        ...
                                        <td><img alt="thumbnail" src="{{ asset('storage/' . $transaction_type->thumbnail) }}"
                                                width="100" height="100"></td>
                                        <td>{{ $transaction_type->code }}</td>
                                        <td>{{ $transaction_type->action == 'dr' ? 'Debit' : 'Credit' }}</td>
                                        <td>
                                            <form
                                                action="{{ route('admin.transaction_types.destroy', $transaction_type->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <a
                                                    href="{{ route('admin.transaction_types.edit', $transaction_type->id) }}">
